...
class NewAltTextManager {
  ...
  constructor(
    { descriptionContainer, dialog, imagePreview, cancelButton, disclaimer, notNowButton, saveButton, textarea, learnMore, errorCloseButton, createAutomaticallyButton, downloadModel, downloadModelDescription, title },
    overlayManager,
    eventBus
  ) {
...
class ImageAltTextSettings {
  ...
  constructor({ dialog, createModelButton, aiModelSettings, learnMore, closeButton, deleteModelButton, downloadModelButton, showAltTextDialogButton }, overlayManager, eventBus, mlManager) {
    this.#dialog = dialog;
...
class AnnotationEditorParams {
  ...
  #bindListeners({ editorFreeTextFontSize, editorFreeTextColor, editorInkColor, editorInkThickness, editorInkOpacity, editorStampAddImage, editorFreeHighlightThickness, editorHighlightShowAll, editorSignatureAddSignature }) {
    const { eventBus } = this;
...
class CommentManager {
  ...
  #dialogY = 0;
  #menuAC = null;
  constructor({ dialog, toolbar, actions, menu, editMenuItem, deleteMenuItem, closeButton, textInput, textView, cancelButton, saveButton }, sidebar, eventBus, linkService, overlayManager) {
...
class CommentSidebar {
  ...
  #idsToElements = null;
  constructor({ sidebar, commentsList, commentCount, sidebarTitle, closeButton, commentToolbarButton }, eventBus, linkService) {
...
class PDFDocumentProperties {
  ...
  async open() {
    ...
    const [{ info, metadata, contentLength }, pdfPage] = await Promise.all([this.pdfDocument.getMetadata(), this.pdfDocument.getPage(currentPageNumber)]);
  ...
  async #parsePageSize(pageSizeInches, pagesRotation) {
    ...
    const [{ width, height }, unit, name, orientation] = await Promise.all([
      ...
      this.l10n.get(isPortrait ? "pdfjs-document-properties-page-size-orientation-portrait" : "pdfjs-document-properties-page-size-orientation-landscape"),
    ]);
...
const DIACRITICS_EXCEPTION = new Set([
  0x3099, 0x309a, 0x094d, 0x09cd, 0x0a4d, 0x0acd, 0x0b4d, 0x0bcd, 0x0c4d, 0x0ccd, 0x0d3b, 0x0d3c, 0x0d4d, 0x0dca, 0x0e3a, 0x0eba, 0x0f84, 0x1039, 0x103a, 0x1714, 0x1734, 0x17d2, 0x1a60, 0x1b44, 0x1baa, 0x1bab, 0x1bf2, 0x1bf3, 0x2d7f,
  0xa806, 0xa82c, 0xa8c4, 0xa953, 0xa9c0, 0xaaf6, 0xabed, 0x0c56, 0x0f71, 0x0f72, 0x0f7a, 0x0f7b, 0x0f7c, 0x0f7d, 0x0f80, 0x0f74,
]);
...
class PDFPrintService {
  ...
  renderPages() {
    ...
    const renderNextPage = (resolve, reject) => {
      ...
      renderPage(this, this.pdfDocument, index + 1, this.pagesOverview[index], this._printResolution, this._optionalContentConfigPromise, this._printAnnotationStoragePromise)
...
class PDFThumbnailView {
  constructor({ container, eventBus, id, defaultViewport, optionalContentConfigPromise, linkService, renderingQueue, maxCanvasPixels, maxCanvasDim, pageColors }) {
    this.id = id;
...
class AnnotationLayerBuilder {
  ...
  async injectLinkAnnotations({ inferredLinks, viewport, structTreeLayer = null }) {
...
class SecondaryToolbar {
  ...
  #scrollModeChanged({ mode }) {
    const { scrollPageButton, scrollVerticalButton, scrollHorizontalButton, scrollWrappedButton, spreadNoneButton, spreadOddButton, spreadEvenButton } = this.#opts;
...
class SignatureManager {
  ...
  async loadSignatures(reload = false) {
    ...
    if (!this.#loadSignaturesPromise) {
      this.#loadSignaturesPromise = this.#signatureStorage.getAll().then(async (signatures) => [signatures, await Promise.all(Array.from(signatures.values(), ({ signatureData }) => SignatureExtractor.decompressSignature(signatureData)))]);
      if (!reload) {
        return;
      }
    }
...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    if (!this.supportsPrinting) {
      ...
      eventBus.on("printingallowed", ({ isAllowed }) => togglePrintingButtons(isAllowed));
    }
